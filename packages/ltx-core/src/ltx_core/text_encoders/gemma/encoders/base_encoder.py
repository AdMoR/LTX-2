...
from transformers import AutoImageProcessor, BitsAndBytesConfig, Gemma3ForConditionalGeneration, Gemma3Processor
...
class GemmaTextEncoderModelBase(torch.nn.Module):
    ...
    def _enhance(
        ...
        with torch.no_grad(), torch.random.fork_rng(devices=[self.model.device]):
